...
public class RabbitConfigTest {
    ...
    private static final String GOOGLE_MEET_QUEUE = "google.meet.queue";
    ...
    private static final String GOOGLE_MEET_ROUTING_KEY = "google.meet.routingKey";
    ...
    @Test
    void testGoogleMeetQueue() {
        when(properties.googleMeetQueue()).thenReturn(GOOGLE_MEET_QUEUE);
        Queue queue = rabbitConfig.googleMeetQueue();
        assertThat(queue.getName()).isEqualTo(GOOGLE_MEET_QUEUE);
    ...
    @Test
    void testGoogleMeetBinding() {
        when(properties.googleMeetRoutingKey()).thenReturn(GOOGLE_MEET_ROUTING_KEY);

        Queue googleQueue = new Queue(GOOGLE_MEET_QUEUE);
        DirectExchange createExchange = new DirectExchange(MEETING_CREATE_EXCHANGE);

        Binding meetBinding = rabbitConfig.googleMeetBinding(googleQueue, createExchange);

        assertThat(meetBinding.getDestination()).isEqualTo(GOOGLE_MEET_QUEUE);
        assertThat(meetBinding.getExchange()).isEqualTo(MEETING_CREATE_EXCHANGE);
        assertThat(meetBinding.getRoutingKey()).isEqualTo(GOOGLE_MEET_ROUTING_KEY);
    }
